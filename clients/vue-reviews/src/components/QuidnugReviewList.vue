...
const sorted = computed(() => {
    ...
    const m = weightMap.value;
    const byWeight = (a, b) => (m.get(b.creator) ?? 0) - (m.get(a.creator) ?? 0);
...
function weightFor(ev) { return weightMap.value.get(ev.creator); }
function isContributing(w) { return w != null && w > 0.1; }
</script>
    ...
    <div v-else style="display:flex;flex-direction:column;gap:12px;margin-top:12px">
        <div v-for="ev in sorted"
             ...
             :style="{
                 ...
                 borderRadius: '6px',
                 background: '#f7f7f7',
                 borderLeft: `3px solid ${isContributing(weightFor(ev)) ? '#2E8B57' : '#ccc'}`,
                 opacity: isContributing(weightFor(ev)) ? 1 : 0.55,
                 fontSize: '14px',
             }">
            <div style="display:flex;gap:8px;align-items:center">
                <span style="color:#F9A825;font-weight:600">★ {{ ev.payload?.rating }}</span>
                <strong v-if="ev.payload?.title">{{ ev.payload.title }}</strong>
                <span :style="{
                          fontSize: '11px',
                          color: isContributing(weightFor(ev)) ? '#2E8B57' : '#999',
                          fontFamily: 'monospace',
                      }">
                    {{ weightFor(ev) != null
                        ? `weight ${weightFor(ev).toFixed(2)}`
                        : 'outside your trust' }}
                </span>
            </div>
            <div style="font-family:monospace;font-size:11px;color:#888;margin:4px 0">
