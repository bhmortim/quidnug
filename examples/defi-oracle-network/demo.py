...
def load_reports(client: QuidnugClient, feed: Actor) -> List[dict]:
    events, _ = client.get_stream_events(feed.quid.id, limit=500)
    out: List[dict] = []
    for ev in events or []:
        out.append({
...
def aggregate_and_show(
    ...
    events = load_reports(client, feed)
...
def main() -> None:
    ...
    aggregate_and_show(client, cons, feed, "BTC-USD", default_policy, "CONSERVATIVE")
    aggregate_and_show(client, perm, feed, "BTC-USD", default_policy, "PERMISSIVE")
